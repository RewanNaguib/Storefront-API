...
const getOrderDetails = async (req: Request, res: Response) => {
  ...
  if (orderId) {
    try {
      const orderDetails: OrderProductType[] = await order.getOrderDetails(
        orderId
      );
  ...
  if (orderId) {
    try {
      if (
        ...
        const addedProductToOrder: OrderProductType =
          await order.addProductToOrder(quantity, orderId, productId);
...
const updateOrderStatusToClosed = async (req: Request, res: Response) => {
  ...
  if (orderId) {
    try {
      const updatedOrderStatusToClosed: OrderType =
        await order.updateOrderStatusToClosed(orderId);

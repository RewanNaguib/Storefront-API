...
const update = async (req: Request, res: Response) => {
  ...
  req.body.userpassword = (await bcrypt.hashSync(
    req.body.userpassword + pepper,
  ...
  if (id) {
    try {
      const updatedUser: UserType = await user.update(
        id,
        userObject.username,
      ...
      if (updatedUser) {
        if (
          ...
          userObject.username.trim().length !== 0 &&
          typeof userObject.useremail == 'string' &&
          userObject.useremail.trim().length !== 0 &&
          typeof userObject.userpassword == 'string' &&
          userObject.userpassword.trim().length !== 0
        ) {
...
const authenticate = async (req: Request, res: Response) => {
  ...
  try {
    if (
      ...
      const loggedInUser: UserType | null = await user.authenticate(
        useremail,
        userpassword
      );
      if (loggedInUser) {

...
export class User{
    ...
    async show(id: string): Promise<UserType>{
        try{
            ...
            throw new Error(`couldn't get user ${id}, Error: ${(error as Error).message}`);
        }
    };
    ...
    async create(u: UserType): Promise<UserType> {
        try {
            const conn = await client.connect();
            const sql  = 'INSERT INTO users (username,useremail,userpassword) VALUES($1, $2, $3) RETURNING *';
            const hash: string = bcrypt.hashSync(u.userpassword + pepper, parseInt(saltRounds as string));
            const result = await conn.query(sql, [u.username, u.useremail, hash]);
            conn.release();
            const user = result.rows[0];
            return user;
        } catch (err) {
            throw new Error(`couldn't add new user ${u.username}, Error: ${(err as Error).message}`);
        }
    };

    // login 
    async authenticate(useremail:string, userpassword:string): Promise<UserType|null>{
        try{
            const conn = await client.connect();
    ...
    async update(id: string, username: string, useremail: string, userpassword: string): Promise<UserType> {
        try{
            const conn = await client.connect();
            const sql = 'UPDATE users SET username=($2), useremail=($3), userpassword=($4) WHERE id=($1) RETURNING *;';
            const result = await conn.query(sql, [id, username, useremail, userpassword]);
            conn.release();
            const user = result.rows[0];
            return user;
        }catch(error){
            throw new Error(`couldn't update user ${id}, Error: ${(error as Error).message}`);
        }
    };
    ...
    async destroy(id: string): Promise<number>{
        try{
            ...
            throw new Error(`couldn't delete user ${id}, Error: ${(error as Error).message}`);
        }
    };

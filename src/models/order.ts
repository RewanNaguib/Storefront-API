...
export class Order{
    ...
    async index(): Promise<OrderType[]>{
        try{
            ...
            throw new Error(`couldn't get orders, Error: ${(error as Error).message}`);
        }
    };
    ...
    async show(id: string): Promise<OrderType>{
        try{
            ...
            throw new Error(`couldn't get order ${id}, Error: ${(error as Error).message}`);
        }
    };
    ...
    async getUserOrders(userId: string): Promise<OrderType[]>{
        try{
            ...
            throw new Error(`couldn't get orders for user ${userId}, Error: ${(error as Error).message}`);
        }
    };
    ...
    async getOrderDetails(orderId: string): Promise<OrderProductType[]>{
        try{
            ...
            throw new Error(`couldn't get details of order ${orderId}, Error: ${(error as Error).message}`);
        }
    };
    ...
    async getUserClosedOrders(userId: string): Promise<OrderType[]>{
        try{
            ...
            throw new Error(`couldn't get closed orders of user ${userId}, Error: ${(error as Error).message}`);
        }
    };
    ...
    async create(id: string, userId: number): Promise<OrderType> {
        try {
            ...
            throw new Error(`couldn't add new order for userId ${userId}, Error: ${(err as Error).message}`);
        }
    };

    // specific user add product(s) to order
    async addProductToOrder(quantity: number, orderId: string, productId: number): Promise<OrderProductType>{
        ...
        try{
            ...
            if(orderstatus !== "open") {
                throw new Error(`could not add product ${productId} to order ${orderId} because order status is ${orderstatus}`);
            }
        } catch(error){
            throw new Error(`order ${orderId} not found, Error:${(error as Error). message}`);
        }
        ...
        try{
            ...
            throw new Error(`can not add product ${productId} to order ${orderId} ${(error as Error).message}`);
        }
    };
    ...
    async updateOrderStatusToClosed(orderId: string): Promise<OrderType> {
        try{
            ...
            throw new Error(`couldn't update status of order ${orderId} to closed, Error: ${(error as Error).message}`);
        }
    };
